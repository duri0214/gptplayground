...
class HomeView(FormView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["chat_logs"] = ChatLogs.objects.filter(user=login_user).order_by('thread', 'created_at')
    ...
    def form_valid(self, form):
        ...
        file_path = str(BASE_DIR / r'retrieval_qa_with_source\tests\domain\valueobject\令和4年版少子化社会対策白書全体版（PDF版）.pdf')
        ...
        ChatLogs.objects.create(user=login_user, thread='XXX', role='user', message=form_data['question'])
        ChatLogs.objects.create(user=login_user, thread='XXX', role='assistant', message=formatted_answer)
